...
{
    // Convert Bond:// to Bond:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Bond://"))
    {
        uri.replace(0, 12, "Bond:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bond.lnk";
}
...
{
    return GetAutostartDir() / "Bond.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bond-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Bond-qt [" + tr("command-line options") + "]                     " + "\n";
